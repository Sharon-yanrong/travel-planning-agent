...
**🔗 在线体验（点开即用）：https://travel-planning-agent-u4yi.onrender.com**
（免费实例，首次打开约 30–50 秒唤醒；之后即时响应。）
...
![主界面](shots/web_home.png)
*出行类型（多画像）+ 我的红线（硬约束）+ 定位：不给完美攻略，给能改的底稿*

![工具调用](shots/web_plan1.png)
*一句需求 → 模型自主调用 天气 / 景点检索 / 路线优化 等工具，并说明「为什么这么排」*

![可编辑底稿](shots/web_plan4.png)
*结构化行程：可删条目、可锁「🔒已订」、可导出（TXT / PDF）、可让 AI 按修改重排*
...
当前版本聚焦「规划」环节的核心价值——懂约束、可执行、能应变,并已完整跑通、上线可用。 
数据深度:现阶段景点库与价格 / 开放时间 / 余票以基础数据为主,下一步接入真实、实时的 POI 与库存数据,让建议更精准可依。
路线精度:当前以经纬度直线距离做全局最优,后续接入真实步行 / 公共交通时长,进一步贴近实际。
业务闭环:打通「规划 → 预订」闭环并引入账户体系,从「帮你想清楚」延伸到「帮你订下来」。
